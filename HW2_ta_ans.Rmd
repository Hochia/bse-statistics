...
$$
\begin{aligned}
F_X(x)&=f_X(12)\cdot(40-5)/2=1\\
f_X(12)&=2/35\\
F_Y(y)&=f_Y(35)\cdot(55-15)/2=1\\
f_Y(35)&=1/20
\end{aligned}
$$
...
$$
\begin{aligned}
F_X(x)&=0.4\cdot2a/2=1,\quad a=2.5
\end{aligned}
$$
...
f_X(x)=
\left\{ \begin{array}{ll}
0.4/2.5\cdot(x-10)  & & 10\leq x\leq 12.5,\\
0.4 - 0.4/2.5\cdot(x-12.5) & & 12.5< x\leq 15,\\
0 & & \text{o.w.}
\end{array} \right.
\end{aligned}
$$
...
f_Y(y)=
\left\{ \begin{array}{ll}
f_X(y)/0.5 = (0.4 - 0.4/2.5\cdot(y-12.5))/0.5 && 12.5< y\leq 15,\\
0 && \text{o.w.}
\end{array} \right.
\end{aligned}
$$
...
&=P\Bigg(\Big(\frac{X_i+1}{2}\Big)=k_i,\ i=1,2,\dots,n\ \Big|\sum^n_{i=1}k_i=k\Bigg)\\
...
&=\sum^{20}_{k=14}{20\choose k}0.6^k(1-0.6)^{20-k}\\
&=0.25
\end{aligned}
$$

```{r}
sum(map_dbl(14:20, ~ choose(20, .x) * 0.6^.x * (1 - 0.6)^{20 - .x}))
```
...
$$
\begin{aligned}
\int^1_{-1}a\sqrt{1-x^2}dx
&=a(\frac{x}{2}\sqrt{1-x^2}+\frac{a^2}{2}\sin^{-1}x)|^1_{-1}\\
&=\frac{a^3}{2}\pi,\\
a&=\sqrt[3]{\frac{2}{\pi}}
\end{aligned}
$$
...
f_Y(y)=
\left\{ \begin{array}{ll}
2\sqrt[3]{\frac{2}{\pi}}\sqrt{1-(x-b)^2} && b\leq x\leq b+1,\\
0 && \text{o.w.}
\end{array} \right.
\end{aligned}
$$
